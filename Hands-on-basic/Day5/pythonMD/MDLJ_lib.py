class MDLJ_lib :
  ...
  def write_velocities(self, N):
      from numpy import zeros
      sig = 3.4  # sigma in Angstrom for Argon
      eps = 120. * 1.38064852e-23  # epsilon in J, kB = 1.38e-23
      mass_kg = 39.948 * 1.66053906660e-27  # mass in kg (argon)

        # Time unit: tau = sigma * sqrt(m/epsilon)
      tau = sig * 1e-10 * (mass_kg / eps)**0.5  # in seconds
      tau_ps = tau * 1e12  # convert to picoseconds

      vscale = sig / tau_ps  # scale to convert reduced velocity to Å/ps

      fout = open('velocities.xyz', 'a')
      fout.write('  %d\n' % N)
      fout.write('\n')
      for i in range(N):
        vx = self.px[i] / (self.mass * self.L) * vscale
        vy = self.py[i] / (self.mass * self.L) * vscale
        vz = self.pz[i] / (self.mass * self.L) * vscale
        fout.write('Ar   %12.5g   %12.5g   %12.5g\n' % (vx, vy, vz))
      fout.close()
